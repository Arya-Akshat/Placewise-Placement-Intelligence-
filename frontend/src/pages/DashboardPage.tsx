      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 pb-4 border-b border-slate-800/80">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1">
            Grounded directly in Unity Catalog (<code className="text-emerald-400 font-mono">placewise.semantic.*</code>) • Batch 2024
          </p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-xs font-semibold text-white shadow-md transition"
          >
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 pt-2">
        ...
        <div className="relative w-full sm:w-64">
          ...
          <input
            ...
            className="w-full pl-8 pr-3 py-1.5 bg-slate-900 border border-slate-800 rounded-lg text-xs text-slate-200 placeholder-slate-500 focus:outline-none focus:border-emerald-500/60"
          />
        </div>
      </div>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-slate-900/90 border border-slate-800 rounded-xl overflow-hidden shadow-sm">
            <div className="px-4 py-3 border-b border-slate-800 flex items-center justify-between">
              ...
              <span className="text-[11px] text-slate-400">{filteredDepartments.length} Departments</span>
            </div>
                  ...
                  {filteredDepartments.map((dept: any, idx: number) => {
                    ...
                    return (
                      <tr key={idx} className="hover:bg-slate-800/40 transition">
                        <td className="py-2.5 px-4 font-bold text-emerald-400">{dept.department_code}</td>
                        <td className="py-2.5 px-4 text-slate-100 font-sans">{dept.department_name}</td>
                        <td className="py-2.5 px-4 font-bold">{dept.placement_rate}%</td>
                        <td className="py-2.5 px-4 text-slate-400">{dept.placed_students} / {dept.eligible_students}</td>
                        <td className="py-2.5 px-4 text-slate-200 font-sans">₹{dept.average_ctc_lpa} LPA</td>
                        <td className="py-2.5 px-4 font-bold">
                          <span className={`px-2 py-0.5 rounded-full text-[10px] ${isPositive ? 'bg-emerald-500/10 text-emerald-400' : 'bg-rose-500/10 text-rose-400'}`}>
                        ...
                        <td className="py-2.5 px-4 text-right">
                          <button
                            ...
                            className="inline-flex items-center gap-1 text-[10px] text-emerald-400 hover:text-emerald-300 underline font-sans"
                          >
                            <span>Analyze</span>
      ...
      {activeTab === 'companies' && (
        <div className="bg-slate-900/90 border border-slate-800 rounded-xl overflow-hidden shadow-sm">
          <div className="px-4 py-3 border-b border-slate-800 flex items-center justify-between">
            ...
            <span className="text-[11px] text-slate-400">{filteredCompanies.length} Employers</span>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs text-slate-300">
              <thead className="bg-slate-950/60 text-slate-400 uppercase text-[10px] tracking-wider border-b border-slate-800">
              ...
              <tbody className="divide-y divide-slate-800/60 font-mono text-[11px]">
                {filteredCompanies.map((comp: any, idx: number) => (
                  <tr key={idx} className="hover:bg-slate-800/40 transition">
                    <td className="py-2.5 px-4 font-bold text-slate-100 font-sans">{comp.company_name}</td>
                    <td className="py-2.5 px-4 text-slate-400 font-sans">{comp.industry}</td>
                    <td className="py-2.5 px-4">
                      <span className="px-2 py-0.5 rounded bg-slate-800 text-[10px] text-slate-300 font-sans">
                        {comp.company_type}
                      </span>
                    </td>
                    <td className="py-2.5 px-4 font-bold text-emerald-400">{comp.placements_count}</td>
                    <td className="py-2.5 px-4 text-slate-200 font-sans">₹{comp.average_ctc_lpa} LPA</td>
                    <td className="py-2.5 px-4 text-slate-300">{comp.interview_to_offer_rate}%</td>
                    <td className="py-2.5 px-4 text-right">
                      <button
      ...
      {activeTab === 'skills' && (
        <div className="bg-slate-900/90 border border-slate-800 rounded-xl overflow-hidden shadow-sm">
          <div className="px-4 py-3 border-b border-slate-800 flex items-center justify-between">
            ...
            <span className="text-[11px] text-slate-400">{filteredSkills.length} Technical Skills</span>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs text-slate-300">
              <thead className="bg-slate-950/60 text-slate-400 uppercase text-[10px] tracking-wider border-b border-slate-800">
              ...
              <tbody className="divide-y divide-slate-800/60 font-mono text-[11px]">
                {filteredSkills.map((sk: any, idx: number) => (
                  <tr key={idx} className="hover:bg-slate-800/40 transition">
                    <td className="py-2.5 px-4 font-bold text-slate-100 font-sans">{sk.skill_name}</td>
                    <td className="py-2.5 px-4 text-slate-400 font-sans">{sk.skill_category}</td>
                    <td className="py-2.5 px-4 font-bold text-emerald-400">{sk.job_posting_count}</td>
                    <td className="py-2.5 px-4 text-slate-200">{sk.market_demand_ratio}%</td>
                    <td className="py-2.5 px-4 text-slate-300">{sk.student_supply_ratio}%</td>
      ...
      {activeTab === 'candidates' && (
        <div className="bg-slate-900/90 border border-slate-800 rounded-xl overflow-hidden shadow-sm">
          <div className="px-4 py-3 border-b border-slate-800 flex items-center justify-between">
            ...
            <span className="text-[11px] text-slate-400">{filteredCandidates.length} Candidates</span>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs text-slate-300">
              <thead className="bg-slate-950/60 text-slate-400 uppercase text-[10px] tracking-wider border-b border-slate-800">
              ...
              <tbody className="divide-y divide-slate-800/60 font-mono text-[11px]">
                {filteredCandidates.map((cand: any, idx: number) => (
                  <tr key={idx} className="hover:bg-slate-800/40 transition">
                    <td className="py-2.5 px-4 text-slate-400">{cand.student_id}</td>
                    <td className="py-2.5 px-4 font-bold text-slate-100 font-sans">{cand.full_name}</td>
                    <td className="py-2.5 px-4 text-emerald-400">{cand.department_code}</td>
                    <td className="py-2.5 px-4">{cand.cgpa}</td>
                    <td className="py-2.5 px-4 font-bold text-emerald-300">{cand.placement_readiness_score} / 100</td>
                    <td className="py-2.5 px-4 font-sans">
                      <span className={`px-2 py-0.5 rounded text-[10px] font-semibold ${
                        cand.readiness_band === 'EXCELLENT' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-blue-500/20 text-blue-400'
                      }`}>
                        {cand.readiness_band}
                      </span>
                    </td>
                    <td className="py-2.5 px-4 font-sans text-slate-400">{cand.placement_status}</td>
                    <td className="py-2.5 px-4 text-right">
                      <button
