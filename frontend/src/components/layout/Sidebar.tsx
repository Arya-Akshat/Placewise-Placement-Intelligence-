...
export const Sidebar: React.FC = () => {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed md:static inset-y-0 left-0 z-40 w-64 bg-white dark:bg-slate-950 border-r border-slate-200 dark:border-slate-800 flex flex-col justify-between transition-all duration-200 ease-in-out ${
      ...
      >
        <div className="p-3">
          <div className="flex items-center justify-between md:hidden mb-2">
            ...
            <button onClick={toggleSidebar} className="p-1 rounded text-slate-500 hover:text-slate-800 dark:text-slate-400 dark:hover:text-white">
              <X className="w-4 h-4" />
            </button>
          </div>
          ...
          <div className="space-y-2">
            <button
              ...
              className={`w-full flex items-center justify-center gap-2 py-2 px-3 rounded-xl font-semibold text-xs shadow-sm transition ${
                currentView === 'dashboard'
                  ? 'bg-emerald-600 text-white shadow-emerald-500/20'
                  : 'bg-slate-100 hover:bg-slate-200 dark:bg-slate-900 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700'
              }`}
            >
              ...
              {domains.map((d, i) => {
                ...
                return (
                  <button
                    ...
                    className="w-full flex items-center gap-2.5 px-2.5 py-1.5 rounded-lg text-xs text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-100 hover:bg-slate-100 dark:hover:bg-slate-900 transition text-left"
                  >
          ...
          <div className="mt-6">
            ...
            <div className="space-y-1 max-h-60 overflow-y-auto pr-1">
              {conversations.length === 0 ? (
                <span className="text-xs text-slate-400 dark:text-slate-600 px-2 italic">No previous chats</span>
              ) : (
                conversations.map(c => (
                  <button
                    ...
                    className={`w-full flex items-center justify-between px-2.5 py-2 rounded-lg text-xs text-left transition ${
                      currentConversationId === c.conversation_id
                        ? 'bg-emerald-50 dark:bg-slate-800 text-emerald-700 dark:text-emerald-300 font-semibold'
                  ...
                  >
                    ...
                    <span className="text-[10px] text-slate-400 dark:text-slate-600 ml-1 shrink-0">
        ...
        {/* Footer */}
        <div className="p-3 border-t border-slate-200 dark:border-slate-800/80 bg-slate-50 dark:bg-slate-950 text-[11px] text-slate-500 transition-colors">
          <div className="flex items-center justify-between">
            <span>Placewise v2.0</span>
            <span className="text-emerald-600 dark:text-emerald-500 font-mono">ONLINE</span>
          </div>
        </div>
      </aside>
    </>
